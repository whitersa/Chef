...
#### 🚀 K8s 极速开发模式 (Recommended)

如果你已安装 [Skaffold](https://skaffold.dev/)，推荐使用以下命令启动全栈环境：

```bash
# 自动启动所有微服务及数据库，并开启实时代码同步（热重载）
...
- **双阶段失败恢复 (2-Phase Recovery)**: 新增失败重试机制。系统会记录同步过程中因网络波动或限流而跳过的页面，在主同步任务结束后自动进行第二轮追加同步（带有 20s 退避延迟），大幅提升数据完整性。
- **智能翻译 (Azure AI Translator)**: 集成 Azure 翻译服务，在同步过程中动态将食材名称转化为中文，解决了原始数据仅支持英文的问题。

### 2. K8s 极速开发与 PDF 渲染引擎

- **Skaffold 热重载**: 引入 Skaffold 开发工作流。代码修改后 1-3 秒内即可同步至运行中的 K8s Pod，跳过昂贵的镜像构建过程，极大提升本地开发效率。
- **PDF 资源自适应**: 修复了 DITA-OT 引擎在 Docker/Linux 环境下的字体缺失与路径解析问题。支持 wqy-zenhei 中文字体，并实现了 XSL-FO 封面图片路径在 Windows/Linux 间的自动切换。
...
| 工具                     | 用途                                                                                   | 备注                     |
| :----------------------- | :------------------------------------------------------------------------------------- | :----------------------- |
| **Skaffold**             | **K8s 自动化引擎**。实现保存代码后自动热部署到 K8s 集群。                              | 推荐安装 (v2.17+)        |
| **Docker Desktop**       | 容器运行环境，提供本地 Kubernetes 节点。                                               | 必须安装                 |
| **pnpm**                 | 高性能 Node.js 依赖管理工具，支持 Monorepo 过滤构建。                                  | 必须安装                 |
...
## 📄 Milestone 10: 文档发布与导出 (Publishing & Export)

**目标**：实现企业级文档生成能力，支持将结构化数据导出为标准格式（PDF）。

### 🧠 涉及知识点 (Knowledge Points)

- **DITA (Darwin Information Typing Architecture)**: 学习结构化内容标准，理解 Topic, Map, Task 等核心概念。
- **XSLT (Extensible Stylesheet Language Transformations)**: 掌握 XML 转换语言，用于定制 PDF 输出样式。
- **FOP (Formatting Objects Processor)**: 理解 Apache FOP 如何将 XSL-FO 转换为 PDF。
- **字体处理**: 解决中文字体在 PDF 生成中的乱码与缺失问题。

### 10.1 DITA-OT 集成

- [x] **环境集成**:
  - [x] 内置 **DITA-OT 4.2** 与 **Java 17**，实现零依赖部署。
  - [x] 封装 `DitaRunnerService` 自动管理构建环境。
- [x] **自定义插件 (`com.chefos.pdf`)**:
  - [x] 开发自定义 Transtype `chefos-pdf`。
  - [x] 实现 **自动同步机制**：开发时修改插件源码，运行时自动同步到 DITA-OT 环境。
  - [x] **中文字体支持**：配置 `font-mappings.xml` 映射微软雅黑/宋体，解决乱码问题。
- [x] **业务对接**:
  - [x] `DitaGeneratorService`：将 `Recipe` 数据动态转换为 DITA XML。
