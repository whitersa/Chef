...
### 1.1 Monorepo 环境搭建
- [ ] **初始化 Turborepo**：使用 `npx create-turbo@latest` 创建项目，包管理器选择 `pnpm`。
- [ ] **清理默认代码**：删除默认的 `apps/web` (Next.js) 和 `apps/docs`，保持 `packages/` 目录结构。
- [ ] **规划目录结构**：
    ```text
    apps/
    ...
    ```
- [ ] **配置工作区 (Workspace)**：在 `pnpm-workspace.yaml` 中确认包含 `apps/*` 和 `packages/*`。
- [ ] **统一工程规范**：
    - [ ] 在根目录配置 `.eslintrc.js` 和 `.prettierrc`。
    - [ ] 配置 `.vscode/settings.json` 实现保存自动格式化。
    - [ ] (可选) 配置 Husky + Commitlint 拦截不规范提交。

### 1.2 共享库开发 (Shared Packages)
- [ ] **创建 `packages/types`**：
    - [ ] 初始化 `package.json` (name: `@chefos/types`)。
    - [ ] 配置 `tsconfig.json` (生成 `.d.ts` 声明文件)。
    - [ ] 创建 `index.ts`，暂时导出一个测试接口 `export interface Ping { message: string }`。
- [ ] **创建 `packages/api-client`**：
    - [ ] 安装 `axios`。
    - [ ] 封装 `AxiosInstance`，配置基础 URL 和 响应拦截器（预留 Token 处理位置）。

### 1.3 数据库环境
- [ ] **本地 Docker 环境**：
    - [ ] 编写 `docker-compose.yml`。
    - [ ] 配置 **PostgreSQL** 服务 (端口 5432)。
    - [ ] 配置 **Redis** 服务 (端口 6379)。
...
### 3.2 可视化编辑器 (Editor)
- [ ] **布局搭建**：左侧“食材库列表”，中间“画布/树形结构”，右侧“属性面板”。
- [ ] **拖拽实现**：
    - [ ] 安装 `vuedraggable` 或 `dnd-kit`。
    - [ ] 实现从左侧拖拽食材到中间，更新本地 JSON 数据结构。
- [ ] **实时计算**：
    - [ ] 利用 Vue `computed` 或 `watch`，当数量变化时，自动计算当前层级的预估成本。

### 3.3 数据可视化 (Dashboard)
- [ ] **ECharts 集成**：
    - [ ] 封装通用 `BaseChart.vue` 组件。
    - [ ] 实现“营养成分饼图”：根据当前配方数据，动态渲染 蛋白质/碳水/脂肪 比例。
